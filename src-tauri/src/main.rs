...
const GLOBAL_SHORTCUT_OPEN: &str = "Ctrl+Shift+O";
const GLOBAL_SHORTCUT_QUIT: &str = "Ctrl+Shift+Q";
...
<ul>
<li>Place it in ~/.config/EvilClippy/scripts.</li>
...
fn load_script(script_command: &str) -> String {
    ...
    let path = PathBuf::from(appdata_dir).join(CLIPPY_SCRIPT_DIR).join(file_name);
