...
To use Clippy, you need to prepare replay scripts. You can find the scripts from the video in [./demo-scripts](demo-scripts). Place them in the appropriate directory:

- **Windows**: `%appdata%/EvilClippy/scripts`
- **Linux**: `~/.config/EvilClippy/scripts`
- **macOS**: `~/Library/Application Support/EvilClippy/scripts`

Then load them by typing `/load script1.tsv` into Clippy. If you want to abort a certain script, wait for any message in progress to finish, then type `/clear` and load another script.
